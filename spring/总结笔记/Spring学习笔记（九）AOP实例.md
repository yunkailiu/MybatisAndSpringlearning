...
- Joinpoint (连接点): 所谓连接点是指那些被拦截到的点。在 Spring 中,这些点指的是方法,因为 Spring 只支持方法类型的连接点。(可以理解为在业务层接口中的方法)
- Pointcut (切入点): 所谓切入点是指我们要对哪些 Joinpoint 进行拦截的定义。(被代理增强的方法)
...
4. 在 aop : aspect 标签的内部使用对应标签来配置通知的类型
    ...
    2. 切入点表达式的写法：
        ...
        - 配置切入点表达式（aop : pointcut）：
            ...
            - 此标签写在 aop : aspect 标签内部只能当前切面使用，在其外部则所有切面可用(注意顺序，要放到aop:config标签之前)
    3. Spring常用通知类型
    ...
    4.  环绕通知(也可以实现前面四种通知)
    5.  基于注解的AOP配置中，前四种通知类型的执行顺序有一些问题，要慎用。
...
代码：springAOP，之前的acount案例添加AOP配置在accountAOP中
